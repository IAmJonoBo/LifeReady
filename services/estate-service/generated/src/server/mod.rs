...
where
    ...
    A: apis::assets::Assets<E, Claims = C>
        + apis::instructions::Instructions<E, Claims = C>
        + apis::people::People<E, Claims = C>
        + apis::roles::Roles<E, Claims = C>
    ...
    C: Send + Sync + 'static,
{
    ...
    Router::new()
        .route("/estate/v1/assets", post(v1_assets_post::<I, A, E, C>))
        .route(
            "/estate/v1/instructions",
...
) -> std::result::Result<(models::AssetCreate,), ValidationErrors> {
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let validation = tokio::task::spawn_blocking(move || v1_assets_post_validation(body))
        .await
        .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .v1_assets_post(&method, &host, &cookies, &claims, &body)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::assets::V1AssetsPostResponse::Status201_Created { body, x_request_id } => {
            ...
            apis::assets::V1AssetsPostResponse::Status400_ErrorResponse { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::assets::V1AssetsPostResponse::Status401_Unauthorized { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::assets::V1AssetsPostResponse::Status403_Forbidden { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::assets::V1AssetsPostResponse::Status409_Conflict { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
...
) -> std::result::Result<(models::InstructionCreate,), ValidationErrors> {
...
where
    I: AsRef<A> + Send + Sync,
    A: apis::instructions::Instructions<E, Claims = C>
        + apis::ApiAuthBasic<Claims = C>
        + Send
        + Sync,
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || v1_instructions_post_validation(body))
        .await
        .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .v1_instructions_post(&method, &host, &cookies, &claims, &body)
        .await;
...
) -> std::result::Result<(models::V1PeopleGetQueryParams,), ValidationErrors> {
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || v1_people_get_validation(query_params))
        .await
        .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .v1_people_get(&method, &host, &cookies, &claims, &query_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::people::V1PeopleGetResponse::Status200_OK { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::people::V1PeopleGetResponse::Status400_ErrorResponse { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::people::V1PeopleGetResponse::Status401_Unauthorized { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::people::V1PeopleGetResponse::Status403_Forbidden { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
...
) -> std::result::Result<(models::PersonCreate,), ValidationErrors> {
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || v1_people_post_validation(body))
        .await
        .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .v1_people_post(&method, &host, &cookies, &claims, &body)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::people::V1PeoplePostResponse::Status201_Created { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::people::V1PeoplePostResponse::Status400_ErrorResponse { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::people::V1PeoplePostResponse::Status401_Unauthorized { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::people::V1PeoplePostResponse::Status403_Forbidden { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::people::V1PeoplePostResponse::Status409_Conflict { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
...
) -> std::result::Result<(models::RoleGrantCreate,), ValidationErrors> {
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || v1_roles_grants_post_validation(body))
        .await
        .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .v1_roles_grants_post(&method, &host, &cookies, &claims, &body)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::roles::V1RolesGrantsPostResponse::Status201_Created { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::roles::V1RolesGrantsPostResponse::Status403_Forbidden { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
            ...
            apis::roles::V1RolesGrantsPostResponse::Status409_Conflict { body, x_request_id } => {
                if let Some(x_request_id) = x_request_id {
                    let x_request_id = match header::IntoHeaderValue(x_request_id).try_into() {
                        Ok(val) => val,
                        Err(e) => {
