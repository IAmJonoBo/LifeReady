    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    "category" => intermediate_rep.category.push(
                        <models::AssetCategory as std::str::FromStr>::from_str(val)
                    ...
                    #[allow(clippy::redundant_clone)]
                    "asset_id" => intermediate_rep.asset_id.push(
        ...
        std::result::Result::Ok(Asset {
            category: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "category missing in Asset".to_string())?,
            label: intermediate_rep
                .label
                .into_iter()
                .next()
                .ok_or_else(|| "label missing in Asset".to_string())?,
            notes: intermediate_rep.notes.into_iter().next(),
            sensitivity: intermediate_rep.sensitivity.into_iter().next(),
            asset_id: intermediate_rep
                .asset_id
                .into_iter()
                .next()
                .ok_or_else(|| "asset_id missing in Asset".to_string())?,
            created_at: intermediate_rep
                .created_at
                .into_iter()
                .next()
                .ok_or_else(|| "created_at missing in Asset".to_string())?,
        })
    }
}
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing AssetCreate".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing AssetCreate".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(AssetCreate {
            category: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "category missing in AssetCreate".to_string())?,
            label: intermediate_rep
                .label
                .into_iter()
                .next()
                .ok_or_else(|| "label missing in AssetCreate".to_string())?,
            notes: intermediate_rep.notes.into_iter().next(),
            sensitivity: intermediate_rep.sensitivity.into_iter().next(),
        })
    }
...
impl std::convert::TryFrom<header::IntoHeaderValue<AssetCreate>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<AssetCreate>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for AssetCreate - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <AssetCreate as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into AssetCreate - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
...
pub struct Instruction {
    ...
    #[serde(rename = "body")]
    #[validate(length(max = 10000), custom(function = "check_xss_string"))]
    pub body: String,
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing Instruction".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    #[allow(clippy::redundant_clone)]
                    "instruction_id" => intermediate_rep.instruction_id.push(
                        <uuid::Uuid as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    ...
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing Instruction".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(Instruction {
            title: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "title missing in Instruction".to_string())?,
            body: intermediate_rep
                .body
                .into_iter()
                .next()
                .ok_or_else(|| "body missing in Instruction".to_string())?,
            sensitivity: intermediate_rep.sensitivity.into_iter().next(),
            instruction_id: intermediate_rep
                .instruction_id
                .into_iter()
                .next()
                .ok_or_else(|| "instruction_id missing in Instruction".to_string())?,
            created_at: intermediate_rep
                .created_at
                .into_iter()
                .next()
                .ok_or_else(|| "created_at missing in Instruction".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<Instruction>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<Instruction>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for Instruction - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <Instruction as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into Instruction - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing InstructionCreate".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing InstructionCreate".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(InstructionCreate {
            title: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "title missing in InstructionCreate".to_string())?,
            body: intermediate_rep
                .body
                .into_iter()
                .next()
                .ok_or_else(|| "body missing in InstructionCreate".to_string())?,
            sensitivity: intermediate_rep.sensitivity.into_iter().next(),
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<InstructionCreate>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<InstructionCreate>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for InstructionCreate - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <InstructionCreate as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into InstructionCreate - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
...
impl std::fmt::Display for Person {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let params: Vec<Option<String>> = vec![
            ...
            self.phone_e164
                .as_ref()
                .map(|phone_e164| ["phone_e164".to_string(), phone_e164.to_string()].join(",")),
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    #[allow(clippy::redundant_clone)]
                    "full_name" => intermediate_rep.full_name.push(
                        <String as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    ...
                    #[allow(clippy::redundant_clone)]
                    "person_id" => intermediate_rep.person_id.push(
                        <uuid::Uuid as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
        ...
        std::result::Result::Ok(Person {
            full_name: intermediate_rep
                .full_name
                .into_iter()
                .next()
                .ok_or_else(|| "full_name missing in Person".to_string())?,
            ...
            person_id: intermediate_rep
                .person_id
                .into_iter()
                .next()
                .ok_or_else(|| "person_id missing in Person".to_string())?,
            created_at: intermediate_rep
                .created_at
                .into_iter()
                .next()
                .ok_or_else(|| "created_at missing in Person".to_string())?,
        })
    }
}
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing PersonCreate".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing PersonCreate".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(PersonCreate {
            full_name: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "full_name missing in PersonCreate".to_string())?,
            email: intermediate_rep.email.into_iter().next(),
            phone_e164: intermediate_rep.phone_e164.into_iter().next(),
            relationship: intermediate_rep.relationship.into_iter().next(),
        })
...
impl std::convert::TryFrom<header::IntoHeaderValue<PersonCreate>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<PersonCreate>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for PersonCreate - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <PersonCreate as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into PersonCreate - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
...
impl RoleGrant {
    ...
    pub fn new(
        person_id: uuid::Uuid,
        role: models::Role,
        scope: models::RoleGrantCreateScope,
        grant_id: uuid::Uuid,
        status: String,
        created_at: chrono::DateTime<chrono::Utc>,
    ) -> RoleGrant {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing RoleGrant".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    "role" => intermediate_rep.role.push(
                        <models::Role as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "scope" => intermediate_rep.scope.push(
                        <models::RoleGrantCreateScope as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "grant_id" => intermediate_rep.grant_id.push(
                        <uuid::Uuid as std::str::FromStr>::from_str(val)
                            .map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    ...
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing RoleGrant".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(RoleGrant {
            person_id: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "person_id missing in RoleGrant".to_string())?,
            role: intermediate_rep
                .role
                .into_iter()
                .next()
                .ok_or_else(|| "role missing in RoleGrant".to_string())?,
            scope: intermediate_rep
                .scope
                .into_iter()
                .next()
                .ok_or_else(|| "scope missing in RoleGrant".to_string())?,
            grant_id: intermediate_rep
                .grant_id
                .into_iter()
                .next()
                .ok_or_else(|| "grant_id missing in RoleGrant".to_string())?,
            status: intermediate_rep
                .status
                .into_iter()
                .next()
                .ok_or_else(|| "status missing in RoleGrant".to_string())?,
            created_at: intermediate_rep
                .created_at
                .into_iter()
                .next()
                .ok_or_else(|| "created_at missing in RoleGrant".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<RoleGrant>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<RoleGrant>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for RoleGrant - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <RoleGrant as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into RoleGrant - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing RoleGrantCreate".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing RoleGrantCreate".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(RoleGrantCreate {
            person_id: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "person_id missing in RoleGrantCreate".to_string())?,
            role: intermediate_rep
                .role
                .into_iter()
                .next()
                .ok_or_else(|| "role missing in RoleGrantCreate".to_string())?,
            scope: intermediate_rep
                .scope
                .into_iter()
                .next()
                .ok_or_else(|| "scope missing in RoleGrantCreate".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<RoleGrantCreate>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<RoleGrantCreate>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for RoleGrantCreate - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <RoleGrantCreate as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into RoleGrantCreate - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing RoleGrantCreateScope".to_string(),
                    )
                }
            };
        ...
        std::result::Result::Ok(RoleGrantCreateScope {
            access_level: intermediate_rep
                .access_level
                .into_iter()
                .next()
                .ok_or_else(|| "access_level missing in RoleGrantCreateScope".to_string())?,
...
impl std::convert::TryFrom<header::IntoHeaderValue<RoleGrantCreateScope>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<RoleGrantCreateScope>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for RoleGrantCreateScope - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <RoleGrantCreateScope as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into RoleGrantCreateScope - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing V1PeopleGet200Response".to_string(),
                    )
                }
            };
        ...
        std::result::Result::Ok(V1PeopleGet200Response {
            items: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "items missing in V1PeopleGet200Response".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<V1PeopleGet200Response>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<V1PeopleGet200Response>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for V1PeopleGet200Response - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <V1PeopleGet200Response as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into V1PeopleGet200Response - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
