...
impl AuditAppend {
    ...
    pub fn new(
        ...
        action: String,
        tier: models::SensitivityTier,
        payload: std::collections::HashMap<String, crate::types::Object>,
    ) -> AuditAppend {
        AuditAppend {
            actor_principal_id,
            action,
...
impl std::str::FromStr for AuditAppend {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing AuditAppend".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    "actor_principal_id" => intermediate_rep.actor_principal_id.push(
                    ...
                    "payload" => {
                        return std::result::Result::Err(
                            "Parsing a container in this style is not supported in AuditAppend"
                                .to_string(),
                        )
                    }
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing AuditAppend".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(AuditAppend {
            actor_principal_id: intermediate_rep
                .actor_principal_id
                .into_iter()
                .next()
                .ok_or_else(|| "actor_principal_id missing in AuditAppend".to_string())?,
            action: intermediate_rep
                .action
                .into_iter()
                .next()
                .ok_or_else(|| "action missing in AuditAppend".to_string())?,
            tier: intermediate_rep
                .tier
                .into_iter()
                .next()
                .ok_or_else(|| "tier missing in AuditAppend".to_string())?,
            case_id: intermediate_rep.case_id.into_iter().next(),
            payload: intermediate_rep
                .payload
                .into_iter()
                .next()
                .ok_or_else(|| "payload missing in AuditAppend".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<AuditAppend>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<AuditAppend>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for AuditAppend - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
            ...
            std::result::Result::Ok(value) => {
                match <AuditAppend as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into AuditAppend - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing AuditEvent".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    #[allow(clippy::redundant_clone)]
                    "prev_hash" => intermediate_rep.prev_hash.push(
                        <String as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "event_hash" => intermediate_rep.event_hash.push(
                        <String as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing AuditEvent".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(AuditEvent {
            event_id: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "event_id missing in AuditEvent".to_string())?,
            created_at: intermediate_rep
                .created_at
                .into_iter()
                .next()
                .ok_or_else(|| "created_at missing in AuditEvent".to_string())?,
            prev_hash: intermediate_rep
                .prev_hash
                .into_iter()
                .next()
                .ok_or_else(|| "prev_hash missing in AuditEvent".to_string())?,
            event_hash: intermediate_rep
                .event_hash
                .into_iter()
                .next()
                .ok_or_else(|| "event_hash missing in AuditEvent".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<AuditEvent>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<AuditEvent>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for AuditEvent - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <AuditEvent as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into AuditEvent - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                ...
                None => {
                    return std::result::Result::Err(
                        "Missing value while parsing AuditExport".to_string(),
                    )
                }
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    #[allow(clippy::redundant_clone)]
                    "head_hash" => intermediate_rep.head_hash.push(
                        <String as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    #[allow(clippy::redundant_clone)]
                    "events_sha256" => intermediate_rep.events_sha256.push(
                        <String as std::str::FromStr>::from_str(val).map_err(|x| x.to_string())?,
                    ),
                    _ => {
                        return std::result::Result::Err(
                            "Unexpected key while parsing AuditExport".to_string(),
                        )
                    }
                }
            }
        ...
        std::result::Result::Ok(AuditExport {
            download_url: intermediate_rep
                ...
                .into_iter()
                .next()
                .ok_or_else(|| "download_url missing in AuditExport".to_string())?,
            expires_at: intermediate_rep
                .expires_at
                .into_iter()
                .next()
                .ok_or_else(|| "expires_at missing in AuditExport".to_string())?,
            head_hash: intermediate_rep
                .head_hash
                .into_iter()
                .next()
                .ok_or_else(|| "head_hash missing in AuditExport".to_string())?,
            events_sha256: intermediate_rep
                .events_sha256
                .into_iter()
                .next()
                .ok_or_else(|| "events_sha256 missing in AuditExport".to_string())?,
        })
    }
}
...
impl std::convert::TryFrom<header::IntoHeaderValue<AuditExport>> for HeaderValue {
    ...
    fn try_from(
        hdr_value: header::IntoHeaderValue<AuditExport>,
    ) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
            std::result::Result::Ok(value) => std::result::Result::Ok(value),
            std::result::Result::Err(e) => std::result::Result::Err(format!(
                r#"Invalid header value for AuditExport - value: {hdr_value} is invalid {e}"#
            )),
        }
    }
}
        ...
        match hdr_value.to_str() {
            std::result::Result::Ok(value) => {
                match <AuditExport as std::str::FromStr>::from_str(value) {
                    ...
                    std::result::Result::Err(err) => std::result::Result::Err(format!(
                        r#"Unable to convert header value '{value}' into AuditExport - {err}"#
                    )),
                }
            }
            std::result::Result::Err(e) => std::result::Result::Err(format!(
